...
def main():
    ...
    # manager.model.load_state_dict(pretrained_state_dict)

    semirobust_error = manager.eval()
    semirobust_acc = 100 - semirobust_error[0]
    print("Zero-shot semirobust accuracy: ", semirobust_acc)
    ...
    for i in range(10):                               # Number of trials
        ...
        if baseline_acc-best_test_acc > delta: 
            t = time.time()
            trial_test_acc, break_epoch = manager.train(epochs=1, optimizer=optimizer, target_accuracy=best_test_acc, delta=delta, save=False)   
            t = time.time() - t
